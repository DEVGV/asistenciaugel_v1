...
import { Plus, Pencil, Trash2, Phone, Smartphone, ArrowDownCircle } from 'lucide-vue-next';
import { ref } from 'vue';
import TelefonoIEController from '@/actions/App/Http/Controllers/InstitucionEducativa/TelefonoIEController';
...
function submitForm() {
    if (isEditing.value && editingId.value) {
        form.put(TelefonoIEController.update({ telefono: editingId.value }).url, {
            onSuccess: () => closeModal(),
        });
    } else {
        form.post(TelefonoIEController.store({ institucione: props.institucionId }).url, {
            onSuccess: () => closeModal(),
        });
    }
}
...
function executeDelete() {
    ...
    router.delete(TelefonoIEController.destroy({ telefono: itemToDelete.value.id }).url, {
...
function executeBaja() {
    ...
    isBaja.value = true;
    router.patch(TelefonoIEController.darDeBaja({ telefono: itemToBaja.value.id }).url, {}, {
...
<template>
    <div>
            ...
            <Table>
                ...
                <TableBody>
                    <TableRow v-for="tel in telefonos" :key="tel.id">
                        ...
                        <TableCell><StatusBadge :active="!tel.fechaFin" /></TableCell>
                        <TableCell class="text-right">
                            <Button variant="ghost" size="icon" class="h-7 w-7" @click="openEdit(tel)">
                                <Pencil class="h-3.5 w-3.5" />
                            </Button>
                            <Button v-if="!tel.fechaFin" variant="ghost" size="icon" class="h-7 w-7 text-amber-600" title="Dar de baja" @click="confirmBaja(tel)">
        ...
        <ConfirmModal
            ...
            :description="`¿Dar de baja el número ${itemToBaja?.numero}?`"
