...
import { ref } from 'vue';
import DomicilioIEController from '@/actions/App/Http/Controllers/InstitucionEducativa/DomicilioIEController';
...
function submitForm() {
    if (isEditing.value && editingId.value) {
        form.put(DomicilioIEController.update({ domicilio: editingId.value }).url, {
            onSuccess: () => closeModal(),
        });
    } else {
        form.post(DomicilioIEController.store({ institucione: props.institucionId }).url, {
            onSuccess: () => closeModal(),
        });
    }
}
...
function executeDelete() {
    ...
    router.delete(DomicilioIEController.destroy({ domicilio: itemToDelete.value.id }).url, {
...
function executeBaja() {
    ...
    isBaja.value = true;
    router.patch(DomicilioIEController.darDeBaja({ domicilio: itemToBaja.value.id }).url, {}, {
        onSuccess: () => { showBajaModal.value = false; itemToBaja.value = null; },
        onFinish: () => { isBaja.value = false; },
    });
}
</script>
...
<template>
    <div>
            ...
            <Table>
                ...
                <TableBody>
                    <TableRow v-for="item in domicilios" :key="item.id">
                        ...
                        <TableCell><StatusBadge :active="!item.fechaFin" /></TableCell>
                        <TableCell class="text-right">
                            <Button variant="ghost" size="icon" class="h-7 w-7" @click="openEdit(item)">
                                <Pencil class="h-3.5 w-3.5" />
                            </Button>
                            <Button v-if="!item.fechaFin" variant="ghost" size="icon" class="h-7 w-7 text-amber-600" title="Dar de baja" @click="confirmBaja(item)">
                    ...
                    <TableRow v-if="domicilios.length === 0">
                        <TableCell colspan="4" class="h-16 text-center text-sm text-muted-foreground">
            ...
            <div class="grid gap-6">
                <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                    <div class="grid gap-2 md:col-span-2">
                        ...
                        <Input v-model="form.domicilio" placeholder="Av. / Jr. / Calle..." :class="{ 'border-destructive': form.errors.domicilio }" />
                    ...
                    <div class="grid gap-2">
                        <ParamSelect
                            ...
                            @update:modelValue="() => { selectedProvincia = null; selectedDistrito = null; form.ubigeo = ''; }"
                        />
                    </div>
                </div>
                <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                    <div class="grid gap-2">
                        <ParamSelect
                            ...
                            @update:modelValue="() => { selectedDistrito = null; form.ubigeo = ''; }"
                        />
                    </div>
                    <div class="grid gap-2">
                        <ParamSelect
                            ...
                            @update:item="(item: any) => { form.ubigeo = item ? item.codigo || '' : ''; }"
                        />
                    </div>
                    <div class="grid gap-2">
                        <ZonaSelect
                ...
                <div class="grid grid-cols-1 items-end gap-4 border-t pt-4 md:grid-cols-5">
                    <div class="grid gap-2 md:col-span-1">
                        <Label class="text-xs font-bold text-muted-foreground uppercase">Ubigeo</Label>
                        <Input v-model="form.ubigeo" placeholder="000000" maxlength="6" class="h-9 bg-muted/30 font-mono font-bold" />
                    </div>
                </div>
            </div>
        </FormModal>
        ...
        <ConfirmModal
            ...
            description="¿Dar de baja este domicilio?"
